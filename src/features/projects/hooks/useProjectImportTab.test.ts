...
describe('useProjectImportTab - EcoTaxa linked state', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        mockedGetImportableRawSamples.mockResolvedValue([]);
        mockedGetImportableEcoTaxaSamples.mockResolvedValue([]);
    ...
    it('sets hasEcoTaxaProject = true when project.ecotaxa_project_name is present', async () => {
        mockedGetProjectById.mockResolvedValue({
            project_id: 1,
            project_title: 'P',
            project_acronym: 'P',
            instrument_model: 'UVP',
            ecotaxa_project_name: 'EcoProj',
            root_folder_path: '/tmp'
        } as any);

        const { result } = renderHook(() => useProjectImportTab(1));

        await waitFor(() => {
            expect(result.current.loadingRaw).toBe(false);
        });

        expect(result.current.hasEcoTaxaProject).toBe(true);
    });

    it('sets hasEcoTaxaProject = false when project.ecotaxa_project_name is null', async () => {
        mockedGetProjectById.mockResolvedValue({
            project_id: 2,
            project_title: 'P2',
            project_acronym: 'P2',
            instrument_model: 'UVP',
            ecotaxa_project_name: null,
            root_folder_path: '/tmp2'
        } as any);

        const { result } = renderHook(() => useProjectImportTab(2));

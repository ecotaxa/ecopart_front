...
import type { ImportableRawSample, ImportableCtdSample } from "../api/projects.api";
...
export const ProjectImportTab: React.FC<ProjectImportTabProps> = ({ projectId }) => {
    ...
    const rawSamplesColumns: GridColDef<ImportableRawSample>[] = [
        ...
        { field: "raw_file_name", headerName: "Raw file name", flex: 1.5, minWidth: 150, valueGetter: (_value, row) => row.raw_file_name ?? "—" },
        { field: "station_id", headerName: "Station ID", flex: 1, minWidth: 100, valueGetter: (_value, row) => row.station_id ?? "—" },
        { field: "first_image", headerName: "First image frame", flex: 1, minWidth: 100, valueGetter: (_value, row) => row.first_image ?? "—" },
        { field: "last_image", headerName: "Last image frame", flex: 1, minWidth: 100, valueGetter: (_value, row) => row.last_image ?? "—" },
        {
            field: "images_count",
            headerName: "Images",
            flex: 0.8,
        ...
        { field: "comment", headerName: "Comment", flex: 2, minWidth: 150, valueGetter: (_value, row) => row.comment ?? "—" },
    ];
    ...
    const ctdSamplesColumns: GridColDef[] = [
        ...
        { field: "station_id", headerName: "Station ID", flex: 1, minWidth: 120, valueGetter: (_value, row) => row.station_id ?? "—" },
    ];
    ...
    return (
        <>
            ...
            <QcReviewDialog
                ...
                sampleNames={qcSampleNames}
                previews={qcPreviews}
                notImportable={qcNotImportable}
                loading={loadingQcPreview}
                error={qcPreviewError}
                isImporting={isImporting}
                onRemoveSample={removeQcSample}
                onImportValidated={() => confirmAndExecuteRawImport(true)}
                onImportPending={() => confirmAndExecuteRawImport(false)}
            />
